...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['Damn Daniel', 0.5], //From 40% to 49%
		['What', 0.6], //From 50% to 59%
		['Not Good', 0.69], //From 60% to 68%
		['N  I  C  E', 0.7], //69%
		['Not Bad', 0.8], //From 70% to 79%
		['Doin Great', 0.9], //From 80% to 89%
		['Easter!', 1], //From 90% to 99%
		['YESS!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingName == '?') {
			scoreTxt.text = 'Easter Eggs: ' + songScore + ' | Oopsies: ' + songMisses + ' | Grade: ' + ratingName;
		} else {
			scoreTxt.text = 'Easter Eggs: ' + songScore + ' | Oopsies: ' + songMisses + ' | Grade: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - ' + ratingFC;//peeps wanted no integer rating
		}
